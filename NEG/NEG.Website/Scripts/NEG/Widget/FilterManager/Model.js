NEG.Module("NEG.Widget.FilterManager.Model", function (require) {
    ...
    var events = {
        "AFTER_ADD_FILTERS": "Widget_After_Add_Filters",
        "AFTER_REMOVE_FILTERS": "Widget_After_Remove_Filters",
        "AFTER_RESET_FILTERS": "Widget_After_Reset_Filters",
        "AFTER_REPLACE_FILTERS": "Widget_After_Replace_Filters"
    };

    var checkConfig = function (config) {
        var result;
        result = NEG.utility.isDefined(config.filterParas) && config.filterParas.length > 0;
        NEG.utility.Array.each(config.filterParas, function (i, obj) {
            if (!NEG.utility.isType(obj, "String") && NEG.utility.String.trim(obj) != "") {
                result = false;
            }
        });
        return result;
        ...
        for (var i = 0; i < tempFilters.length; i++) {
            ...
            if (checkFilter(filter)) {
                var theFilter = (function () {
                    var res;
                    var index = NEG.utility.Array.indexOf(addedPara, filter.para);
                    if (index == -1) {
                ...
                if (NEG.utility.isDefined(filter.value)) {
                    if (NEG.utility.Array.indexOf(theValuesAdded, filter.value) == -1) {
                    ...
                    for (var k = 0; k < filter.values.length; k++) {
                        ...
                        if (NEG.utility.isDefined(theValue.value) && theValue.value !== "") {
                            if (NEG.utility.Array.indexOf(theValuesAdded, theValue) == -1) {
    ...
    var FilterManager = function (config, option) {
        ...
        this.addFilters = function (theFilters, fireEvent) {
            if (theFilters) {
                ...
                for (var i = 0; i < filters.length; i++) {
                    ...
                    var index = NEG.utility.Array.indexOf(filterParas, filter.para);
                    ...
                    if (theCacheItem) {
                        ...
                        for (var k = 0; k < filter.values.length; k++) {
                            ...
                            if (NEG.utility.isDefined(toBeRemoveValue)) {
                                ...
                                var theIndex = NEG.utility.Array.indexOf(theCacheItem.values, toBeRemoveValue);
        ...
        this.replaceFilters = function (theFilters, fireEvent) {
            ...
            if (theFilters) {
                ...
                for (var i = 0; i < filters.length; i++) {
                    ...
                    var index = NEG.utility.Array.indexOf(filterParas, theFilter.para);
                    var theCacheItem = filterCache[index];
                    ...
                    if (theCacheItem) {
                        for (var theVal in theCacheItem.valueObjMatch) {
                            ...
                            if (NEG.utility.Array.indexOf(theFilter.allValues, toBeRemoveValue.value) == -1) {
                        ...
                        for (var j = 0; j < theFilter.values.length; j++) {
                            ...
                            if (NEG.utility.isDefined(theCachedValue)) {
                                //update
                                var theIndex = NEG.utility.Array.indexOf(theCacheItem.values, theCachedValue);
                                theCacheItem.values.splice(theIndex, 1);
                                theCacheItem.valueObjMatch[theValue.value] = theValue;
                                theCacheItem.values.push(theValue);
                            } else {
        ...
        this.getFilters = function (para) {
            var result;
            if (para) {
                var index = NEG.utility.Array.indexOf(filterParas, para);
                if (index > -1) {
                    var theCacheItem = filterCache[index];
                    if (theCacheItem) {
