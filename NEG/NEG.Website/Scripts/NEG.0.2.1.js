; (function (Global) {
    ...
    var selfElement = (function () {
        ...
        for (var i = scripts.length - 1 ; i >= 0 ; i--) {
            ...
            if (/NEG.0.2.1.js/.test(theScript.src)) {
                return theScript;
            }
        }
    })();
...
; (function (neg) {
    ...
    */
    ...
    utility.Encoding = encodingHelper;
    ...
    var vc = (function () {
        var versions = [], moduleNames = [];

        return {
            getVersion: function (module) {
                var result = "";
                if (module) {
                    var moduleName = theUtility.String.trim(module);
                    if (moduleName) {
                        moduleName = moduleName.toLowerCase();
                        var index = theUtility.Array.indexOf(moduleNames, moduleName);
                        if (index > -1) {
                    ...
                    for (var i = 0; i < versions.length; i++) {
                        result += (moduleNames[i] + ":" + versions[i]) + "  ";
                    }
                ...
                if (info.length > 0) {
                    for (var i = 0; i < info.length; i++) {
                        ...
                        if (theUtility.isDefined(item.version) && theUtility.isDefined(item.module)) {
                            var moduleName = theUtility.String.trim(item.module);
                            if (moduleName) {
                                moduleName = moduleName.toLowerCase();
                                if (theUtility.Array.indexOf(moduleNames, moduleName) == -1) {
                                    moduleNames.push(moduleName);
                                    versions.push(theUtility.String.trim(item.version))
                                }
                            }
                        }
                    }
